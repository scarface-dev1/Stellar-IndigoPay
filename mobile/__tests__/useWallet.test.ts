...
describe('useWallet', () => {
  ...
  it('loads stored public key on mount', async () => {
    ...
    expect(SecureStore.getItemAsync).toHaveBeenCalledWith('indigopay_stellar_public_key');
  ...
  it('connects with a valid Stellar public key', async () => {
    ...
    expect(SecureStore.setItemAsync).toHaveBeenCalledWith('indigopay_stellar_public_key', VALID_PUBLIC_KEY);
    expect(result.current.publicKey).toBe(VALID_PUBLIC_KEY);
    expect(result.current.error).toBe(null);
  });
  ...
  it('disconnects and clears public key', async () => {
    ...
    expect(SecureStore.deleteItemAsync).toHaveBeenCalledWith('indigopay_stellar_public_key');
    expect(result.current.publicKey).toBe(null);
  });
